...
## VOB ASYM + HV PROXIMITY v1.4 — 2026-08-19 (fix wave: Nagasaki 1:1 parity law + protocol audit)
- Nagasaki = **canonical one-state HV-law engine** (`hvp_maxVolEver`: bar[1] is Nagasaki iff volume[1] > every earlier candle; the lanes read its bar[0] view `volume > hvp_maxVolEver`) — same candle truth as v1.3's `hvp_nagaMax`, no per-study re-implementation. Header EXCEPTIONS (None/None) + MECHANIC tag (M2, offset 0 — no displacement in the lane chain). Title/shorttitle v1.4 (`VOB HVP1.4` / `HVP1.4TICK`), log tags. Everything else byte-identical to v1.3 (main d8970ab); alert line unchanged. Proximity theorem gate D=0 (177,155 sequences) re-proved on v1.4.

## VOB ASYM + HV·DISP·TIER PROXIMITY v1.2 — 2026-08-19 (protocol fix; supersedes v1 / v1.1)
- **Part 5 Rule 2 / Mistake #5:** displacement+FVG is in every lane's chain ⇒ **ONE plotshape per lane at offset -1**; every event aligned to its PLOT BAR = bar[1]: HV tier on bar[1] (`volume[1] == ta.highest(volume,N)[1]`), T1/T2/T3 fire read at [1], displaced candle = bar[1]. The v1.1 two-plot split and its dedup are gone → 16 plotshapes, **35/64**. Nagasaki = canonical one-state HV-law engine. Alert line unchanged (operator v1.3 line). EXCEPTIONS (None/None) + MECHANIC tag in the header. Files `versions/VOB_ASYM_HDTPROX_v1.2.pine` + `tick_friendly/VOB_ASYM_HDTPROX_TICKFRIENDLY_v1.2.pine` (generator `scripts/ind/build_vob_hdtprox_v1_2.py`).

## VOB v11.10 — 2026-08-19 (operator order "fix vob v11.9"; Nagasaki 1:1 parity law; protocol Part 3 ALWAYS-ON)
- **Nagasaki is an independent boolean:** the NAG lane and Nagasaki+ANY lose the **cooldown** gate (v11.9 `f_cd_ok(…, cooldown_bars=30)` removed); **ONE Nagasaki state** (`maxVolEver`, HV-law engine) — the HWS section's second running max (`hw_vol/hw_maxVol`, bar-0 seed) is replaced by the bar[0] view of the same state (conf-gated as before). Default ON unchanged. TICK twin shorttitle capped at 10 chars (`VOB11.10T`). Everything else byte-identical to v11.9 (main b18ef9b == operator Desktop `vob v11.9 SUP OG`). Files `versions/VOB_v11.10.pine` + `tick_friendly/VOB_TICKFRIENDLY_v11.10.pine`.

## VOB v11.11 — 2026-08-19 (RVOL DIRECTIONAL engine parity; operator ruling KC v5.0 = master)
- Census 2026-08-19 (245 studies carry the RVOL DIRECTIONAL engine — SAAB/Kratos · RVOL 1x Bull/Bear · Grand Slam/MOAB): the whole vob family (25 files, incl. the operator's live v11.9) carried the pre-KC 1x/GS ladders. v11.11 = v11.10 with the **1x and GS/MOAB threshold tables = KC COMBO SR v5.0** (SAAB/Kratos ×0.56 and WTC ×2 follow the 1x ladder); **RVOL @ Reg engine (Pentagon/WTC/Hiroshima tables) and the hybrid-momentum ladder UNCHANGED**. Everything else byte-identical to v11.10 (main f8e9f67). Per-TF deltas (1x): 2m 19→18 · 3–5m 17/16/15→13 · 6m 14→13 · 7m 12→11 · 9m 10→11 · 11m 8.4→9 · 15m 8.4→7.5 · 26m 6.9→6.5 · 30m 6.9→6.0 · 1h 5.9→4.5 · 2h 3.0→4.0; GS/MOAB 25–50m 10→20 · 2h 8→10 · 1d 8→7.5.
