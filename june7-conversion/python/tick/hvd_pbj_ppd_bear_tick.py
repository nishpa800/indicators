...
"""BASE HV+D <-> PBJ <-> PPD v1 — BEARISH (36) -> Python (TICK grain). FULL port.
...
The tick module and the time module (../time/hvd_pbj_ppd_bear_time.py) share ONE
core; the only differences are (a) the bar-construction grain fed in here and (b)
the per-TF threshold key tfSec, which on tick uses the Pine nn_TICK_FB=10s fallback
...
def run_on_trades(trades, n_ticks: int, *, params: Params | None = None):
    """trades: (ts_ms, price, size) oldest-first. -> 36-plot fire matrix + levels."""
...
if __name__ == "__main__":
    ...
    print("Python is a Python tick — hvd_pbj_ppd_bear (FULL port, 36 plots)")
