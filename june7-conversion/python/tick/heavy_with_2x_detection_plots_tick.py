"""Python is a Python tick — Heavy Weapons w SAAB Kratos x2 fire-matrix on N-tick bars.

Source: ".../June 7/Tick Friendly conversion/heavy_with_2x_detection_plots_tickfriendly.pine"
        (Pine v5, import TradingView/ta/7). FULL port — every one of the 42 detection
        plots (every plotshape signal in the source) is produced from OHLCV by the
        shared core (_heavy_with_2x_detection_plots_core). NO stub engine layer.
...
RE10023 fix (force "D" on tick) — tick bars never align to clock times so RVOL anchors
to the bar's calendar day, never the tick index.

The tick module and the time module (../time/heavy_with_2x_detection_plots_time.py)
share ONE core; the only differences are (a) the bar-construction grain fed in here
and (b) the per-TF threshold key tfSec, which on tick uses the Pine TICK_FALLBACK_SEC=10s
...
def run_on_trades(trades, n_ticks: int, *, params: Params | None = None):
    """trades: (ts_ms, price, size) oldest-first. -> 42-plot fire matrix + levels."""
...
if __name__ == "__main__":
    ...
    print("Python is a Python tick — heavy_with_2x_detection_plots (FULL port)")
