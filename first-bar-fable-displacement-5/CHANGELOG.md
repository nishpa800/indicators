# First Bar Fable Displacement 5 — Changelog
...
Alert-layer ONLY — no plot, checkbox, or engine logic changed; the `First Bar Fable Displacement 5` engine and all plotshapes are byte-identical to the prior version (helper block + ALERTS block are the sole diffs).
...
**"First Bar Fable Displacement 5"**, shorttitle **"1st BAR FABLE D5"**.

- Operator order (Anish, 2026-07-13): new study identical to First Bar Fable v2 with the
  **"Displacement 9" engine default σ-multiplier set to 5**.
...
- **Exactly 3 diff hunks vs base (machine-verified, transform-manifest built):**
  ...
  3. `i_d9_mult` default `9.0 → 5.0` + label `"(5 strength)"`.
...
- Output sha256: `6dc29a10dd34166177f94a0f7446d1069d1c65304fe20429a7b03dbc1d9b8f3b`.
