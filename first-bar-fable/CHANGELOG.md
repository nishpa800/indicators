...
## v4 — 2026-07-15: STYLE LAYER — white text · 50% transparency · size tiers (L-49 W-INDSTUDY three-pack wave)

**Files:** `versions/FIRST_BAR_FABLE_v4.pine` (time) + `versions/FIRST_BAR_FABLE_NTS_v4.pine` (NTS twin, A7 pair).
plotshape() STYLE layer ONLY — 57 plotshape calls per file retextured; zero engine / alert / checkbox / plot-title / plot-count changes. The `First Bar Fable v2` engine and every alert are byte-identical to the prior version; the per-line diff is EXACTLY the 57 plotshape lines (byte-verified, D=0).
...
## v3 — 2026-07-15: ALERT GRAMMAR v3 (L-49 W-INDSTUDY wave)

**Files:** `versions/FIRST_BAR_FABLE_v3.pine` (time) + `versions/FIRST_BAR_FABLE_NTS_v3.pine` (NTS twin, A7 pair).
Alert-layer ONLY — no plot, checkbox, or engine logic changed; the `First Bar Fable v2` engine and all plotshapes are byte-identical to the prior version (helper block + ALERTS block are the sole diffs).
