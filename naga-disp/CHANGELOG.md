...
## NAGA DISP v2.1 — 2026-08-19 (fix wave: ANISH VERIFICATION PROTOCOL v3.3 audit + Nagasaki 1:1 parity law)
- **Logic, inputs, alert lines BYTE-IDENTICAL to v2 (main 81edb57)** — declared counted edits only: title/shorttitle v2.1 (`NAGA+D2.1` / `N+D2.1TICK`), header **EXCEPTIONS** section (CHECKBOX BYPASSERS None · AGGREGATOR BYPASSERS None), **MECHANICS** tags (section 1 = M7 same-bar; RUN = M1 consecutive streak with reset + last-N count; plot bar bar[1], offset −1), **Rule 6** removal note on the KC RVOL transplant (`and conf` dropped, read at [1] ∧ conf-carrying displacement), protocol vocabulary (bar[N] only: "next bar" / "prior closed bar" removed), canonical-Nagasaki note (engine already the HV-law form). Alert grammar unchanged by operator order 2026-08-19.
